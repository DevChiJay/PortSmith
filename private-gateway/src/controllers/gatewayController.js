...
// Handle API routing for PRIVATE gateway
exports.routeApiRequest = async (req, res, next) => {
  try {
    ...
    // PHASE 2: Enforce visibility - PRIVATE gateway only serves PRIVATE APIs
    if (apiConfig.visibility !== 'private') {
      logger.warn(`Attempt to access ${apiConfig.visibility} API '${apiName}' from private gateway`);
      return res.status(404).json({
        error: 'Not Found',
        message: `API '${apiName}' not found`
      });
